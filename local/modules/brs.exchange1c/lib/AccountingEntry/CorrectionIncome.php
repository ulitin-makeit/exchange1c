<?php
	...
	use Brs\FinancialCard\FinancialCard;
	...
	class CorrectionIncome extends \Brs\Exchange1c\AccountingEntry {

		public const ENTITY = 'CORRECTION_INCOME';
		public const ENTITY_NAME = '13. Проводка корректировка поступления';
		...
		/** @var array Список фин карт для каторых данная проводка недоступна */
		private const EXCLUDED_FIN_CARD_TYPE = [
		...
		protected function prepareHandlerData($data1, $data2 = null): void {
			...
			$isPossibleRun = $this->hasCardCorrectionByEventFinCardUpdate($data1) && $this->hasValidSchemeWork($data1);
	...
	//	protected function getSupplierPenalty($price): array
	//	{
	//		return [
	//			'OWN_SERVICE' => true,
	//			'NOMENCLATURE' => Uid::getNomenclatureUid(),
	//			'SERVICE_CONTENT' => 'Штраф поставщика',
	//			'NUMBER' => 1,
	//			'PRICE' => $price,
	//			'AMOUNT' => $price,
	//			'VAT' => ReceiptManager::VAT['VAT_20'],
	...
	//	}
		...
		protected function hasValidSchemeWork(Event $data): bool
		{
			$params = $data->getParameters();
			$finCard = $params['object'];
			$schemeWork = $finCard->get('SCHEME_WORK');

			$isExcludedScheme = in_array($schemeWork, self::EXCLUDED_FIN_CARD_TYPE);

			return !$isExcludedScheme;
		}
	}